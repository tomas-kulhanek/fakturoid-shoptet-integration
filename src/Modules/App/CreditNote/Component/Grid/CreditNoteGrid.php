 ...
 */
class CreditNoteGrid extends Control
{
	...
	protected function createComponentPageGrid(): DataGridControl
	{
		...
		$grid->addColumnDateTime('creationTime', 'messages.creditNoteList.column.creationTime')
			->setFormat('d.m.Y H:i')
			->setSortable();
		$grid->addColumnText('orderCode', 'messages.creditNoteList.column.orderCode')
			->setSortable();
		$grid->addColumnText('proformaInvoiceCode', 'messages.creditNoteList.column.proformaInvoiceCode')
			->setSortable()
			->setDefaultHide(true);
		$grid->addColumnText('accountingNumber', 'messages.creditNoteList.column.accountingNumber')
			->setSortable();
		$grid->addColumnDateTime('changeTime', 'messages.creditNoteList.column.changeTime')
			->setFormat('d.m.Y H:i')
			->setSortable()
			->setDefaultHide(true);
		$grid->addColumnDateTime('accountingUpdatedAt', 'messages.creditNoteList.column.accountingUpdatedAt')
			->setFormat('d.m.Y H:i')
			->setSortable()
			->setDefaultHide(true);
		$grid->addColumnDateTime('dueDate', 'messages.creditNoteList.column.dueDate')
			->setFormat('d.m.Y')
			->setSortable();
		$grid->addColumnDateTime('taxDate', 'messages.creditNoteList.column.taxDate')
			->setFormat('d.m.Y')
			->setSortable();
		$grid->addColumnText('billingAddress.fullName', 'messages.creditNoteList.column.billingFullName')
			->setSortable();
		$grid->addColumnNumber('withVat', 'messages.creditNoteList.column.withVat', 'mainWithVat')
			->setSortable()
			->setRenderer(fn (Document $order) => $this->numberFormatter->__invoke($order->getWithVat(), $order->getCurrencyCode()));

		$grid->addGroupAction(
			'messages.creditNoteList.uploadToAccounting'
		)->onSelect[] = function (array $ids): void {
			$results = ['success' => [], 'error' => []];
			foreach ($ids as $id) {
				$invoice = $this->creditNoteManager->find($this->securityUser->getProjectEntity(), $id);
				bdump($invoice);
				try {
			...
			if (count($results['success']) > 0) {
				$this->getPresenter()->flashSuccess(
					$this->translator->translate(
						'messages.creditNoteList.message.massUploadToAccounting.success',
						[
		...
		$grid->addFilterText('proformaInvoiceCode', 'messages.creditNoteList.column.proformaInvoiceCode');

		$grid->addFilterDateRange('changeTime', 'messages.creditNoteList.column.changeTime');
		$grid->addFilterDateRange('dueDate', 'messages.creditNoteList.column.dueDate');
		$grid->addFilterDateRange('taxDate', 'messages.creditNoteList.column.taxDate');
	...
	public function handleSynchronize(int $id): void
	{
		...
		try {
			...
			Debugger::log($exception);
			$this->getPresenter()->flashError($this->translator->translate('messages.creditNoteList.message.synchronize.error', ['code' => $entity->getCode()]));
		}
		if ($this->getPresenter()->isAjax()) {
			$this->getPresenter()->redrawControl('flashes');

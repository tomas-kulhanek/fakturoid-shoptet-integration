...
class InvoiceSynchronizeCommand extends Command
{
	...
	protected function execute(InputInterface $input, OutputInterface $output): int
	{
		...
		$response = $this->client->getInvoiceChanges($project, $loadFrom);
		...
		while ($response->paginator->totalCount > $total) {
			$response = $this->client->getInvoiceChanges($project, $loadFrom, ($response->paginator->page + 1));
		...
		$project->setLastInvoiceSyncAt(new \DateTimeImmutable());
